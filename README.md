...
担当タスク（担当者に「小林」を含む・状況が空欄/済以外・工数>0の案件）を読み取り、今週の空き時間へ
自動配置する（デフォルトのデータ取得元）。Outlook 予定の Body に `[auto-sched]` マーカー + 段取り/
直近の動き情報を記載するため、日報へ自動転記される。
...
Web化システムを使わず旧来の進捗管理Excel（D列=小林, H列≠済, I列=工数）を直接読みたい場合は
`-Excel` を付ける。
