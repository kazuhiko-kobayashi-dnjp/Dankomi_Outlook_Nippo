...
```mermaid
flowchart LR
    A[露出制御業務進捗管理<br/>manage_exp_progress/Web] -->|st.ps1| B[Outlook 予定登録<br/>段取り/直近の動きを Body に記載]
...
| 時間帯 | 作業 | コマンド |
|---|---|---|
| 週初め | 業務進捗管理タスクを Outlook に段取り | `.\st.ps1 -Execute` |
...
| `st.ps1` | 露出制御業務進捗管理(Web) タスク → Outlook 段取り登録 | `.\st.ps1` / `.\st.ps1 -Execute` / `.\st.ps1 -Clear -Execute` / `.\st.ps1 -Excel` |
...
`st.ps1` で登録した予定は Body に **段取り(plan)・直近の動き(recent)情報**（`-Excel` 指定時はExcelの
E列・F列情報）が含まれ、日報の `＜業務実績＞` 欄へ自動転記される。
...
[露出制御業務進捗管理](http://10.41.55.204:3100/) (manage_exp_progress, Web化システム) の
担当タスク（担当者名に指定した文字列を含む・状況が空欄/済以外・工数>0の案件）を読み取り、今週の空き時間へ
自動配置する（デフォルトのデータ取得元）。担当者名は既定で「小林」だが、配布先の人は後述の
「配布先の人が自分の担当者名で使う場合」の設定で自分の名字に切り替えられる。Outlook 予定の Body に
`[auto-sched]` マーカー + 段取り/直近の動き情報を記載するため、日報へ自動転記される。
...
- Flaskサーバー（`python server.py`）が起動している必要はない。`manage_exp_progress\app\data\tasks.json`
  を直接読み込むだけなので、サーバー停止中でも動作する（本番サーバー10.41.55.204とOneDrive経由で
  同じファイルを共有しているため、内容は常に最新）。
- 段取り(plan)・直近の動き(recent)欄はHTML(リッチテキスト)からプレーンテキストへ自動変換される。
  画像を貼り付けている場合、`<img>`タグごと除去され本文には影響しない。

### データ取得元を旧・進捗管理Excelに戻す

Web化システムを使わず旧来の進捗管理Excel（D列=担当者名, H列≠済, I列=工数）を直接読みたい場合は
`-Excel` を付ける。

```powershell
.\st.ps1 -Excel            # Excelから今週分プレビュー（dry-run）
.\st.ps1 -Excel -Execute   # Excelから Outlook へ登録
.\st.ps1 -Excel 260623 -Execute
```

### 配布先の人が自分の担当者名で使う場合

`st.ps1`（`schedule_tasks.py`）は既定で担当者フィルタ「小林」・`tasks.json`のリポジトリ相対パスを
使うため、そのまま実行すると自分のタスクが拾えない/パスが見つからない場合がある。
`wr.ps1` と同じ `tools/.user_config.json`（Git管理対象外）で担当者名・パスを個人設定できる。

```powershell
# 1) tools\.user_config.example.json を tools\.user_config.json にコピー
# 2) task_owner（自分の名字）/ task_json_path / task_excel_path を自分の値に書き換え
# もしくは担当者名だけをコマンドで保存:
.\st.ps1 -Person "自分の名字" -SavePerson

# その場限りで上書きしたい場合（保存しない）:
.\st.ps1 -Person "自分の名字"
.\st.ps1 -Json "C:\path\to\tasks.json"
.\st.ps1 -Excel -File "C:\path\to\進捗.xlsx"
```

#### OneDriveのローカル同期パスにアクセスできない場合（HTTP経由で直接取得）

`manage_exp_progress`フォルダをOneDrive経由で自分のPCに同期していない人向けに、
社内ネットワーク経由でサーバーへ直接HTTPアクセスして`tasks.json`相当のデータを取得するモードもある。

```powershell
.\st.ps1 -Url "http://10.41.55.204:3100/api/tasks"
```

`-Url`を指定すると`-Json`（ローカルパス）より優先される。毎回指定しなくてよいように
`tools\.user_config.json`の`task_json_url`に保存しておくこともできる。
...
Excelファイルを直接読む代わりに、[露出制御業務進捗管理](manage_exp_progress/)（Web化システム）の
`data/tasks.json` から業務進捗表を読み込むこともできる（`st.ps1`のデフォルトと同じデータソース）。

```powershell
.\wr.ps1 -Summarize -TaskWeb                          # 既定の相対パスから自動検出
.\wr.ps1 -Summarize -TaskWeb -TaskJson "C:\path\to\tasks.json"  # パスを明示指定
```

- Flaskサーバー（`python server.py`）が起動している必要はない。`tasks.json` を直接読むだけ。
- 段取り(plan)・進捗(recent)欄はリッチテキスト(HTML)からプレーンテキストへ自動変換される。
  画像を貼り付けている場合、`<img>` タグごと除去されるため要約には影響しない。
